...
const index = () => {
    ...
    const generaFirmaCliente = async (id) => {
        const { data } = await axiosPrivate.get(`/beneficiario/${id}/firma`)
    ...
    if (tipoPersona == 'N') {
        return <FormNatural firma={'S'} revision={'S'} hash={hash}/>
    }

    if (tipoPersona == 'J') {
        return <FormJuridico firma={'S'} revision={'S'} hash={hash}/>
    }
