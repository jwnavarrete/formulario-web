...
import { validationSchema } from "@utils/Validator/Empleado/Otros/Certificaciones";
...
// 
const index = ({ countCertificaciones }) => {
    ...
    try {
      ...
      if (param.id === undefined) {
        ...
        const { data } = await axiosPrivate.patch(
          `/empleado/certificaciones/${param.id}`,
          param
        );
  ...
  const DelefeRef = async () => {
    ...
    const { data } = await axiosPrivate.delete(
      `/empleado/certificaciones/${codigo}`
    );
    // 
    getRows();
    clearData();
