...
import { validationSchema } from "@utils/Validator/Empleado/Otros/ExperienciaLaboral";
...
const index = ({ countReferencias }) => {
  ...
  const getRows = async () => {
    ...
    const { data } = await axiosPrivate.get(
      `/empleado/experiencia-laboral/${hash}`
    );
  ...
  const prepareData = (data) => {
    ...
    data.fecha_salida = formatDate(data.fecha_salida);
    ...
    try {
      ...
      if (param.id === undefined) {
        ...
        const { data } = await axiosPrivate.patch(
          `/empleado/experiencia-laboral/${param.id}`,
          param
        );
  ...
  const DelefeRef = async () => {
    ...
    const { data } = await axiosPrivate.delete(
      `/empleado/experiencia-laboral/${codigo}`
    );
    // 
    getRows();
    clearData();
  ...
  return (
    <FormProvider {...methods}>
      <Form onSubmit={onSubmit} editMode={editMode} clearData={clearData} DelefeRef={DelefeRef} openModal={openModal} setOpenModal={setOpenModal}/>
