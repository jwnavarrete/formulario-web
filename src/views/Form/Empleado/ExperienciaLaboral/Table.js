...
const Table = ({ rows, setEditMode, setOpenModal }) => {
    ...
    const onRowsSelectionHandler = (data) => {
        ...
        setValue("id", row.id);
        setValue("nombre_empresa", row.nombre_empresa);
        setValue("cargo", row.cargo);
        setValue("motivo_salida", row.motivo_salida);
        setValue("fecha_ingreso", row.fecha_ingreso);
        setValue("fecha_salida", row.fecha_salida);
        // 
        setEditMode(true);
        setOpenModal(true);
    }
