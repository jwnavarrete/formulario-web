...
const index = ({ countReferencias }) => {
  const { hash } = useSelector((state) => state.formJuridico);
    ...
    try {
      ...
      if (param.id === undefined) {
        ...
        const { data } = await axiosPrivate.post(
          `prestador-juridico/referencias-prestador/`,
          param
        );
        ...
        const { data } = await axiosPrivate.patch(
          `prestador-juridico/referencias-prestador/${param.id}`,
          param
        );
  ...
  const DelefeRef = async () => {
    ...
    const { data } = await axiosPrivate.delete(
      `prestador-juridico/referencias-prestador/${codigo}`
    );
    // 
    getRows();
    clearData();
