...
const index = () => {
    const { formData, handleChangeTipSolic, emiFactSolicitante } = useContext(NaturalContext)

    const {watch,getValues} = useFormContext();

useEffect(()=>{

    handleChangeTipSolic(getValues('solicitante'))
    ...
    return (
        ...
        <>
          ...
          <Grid item xs={12} md={6} lg={3}>
            <Input name="telefono_fac" inputProps={{ maxLength: 13 }} label="Tel&eacute;fono" fullWidth />
          </Grid>

          <Grid item xs={12} md={6} lg={6}>
            <Input name="direccion_fac" label="Direcci&oacute;n" fullWidth />
          </Grid>
