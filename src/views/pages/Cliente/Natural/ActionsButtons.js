...
const Botones = ({ grabaDatos }) => {
  ...
  const getData = async () => {
    ...
    var infoDetail = {
      firmante: solicitante.apellidos + ' ' + solicitante.nombres,
  ...
  const firmarFormulario = async () => {
    try {
      ...
      if (formulario.correo_agente) {
        ...
        if (data) {
          const { correo_cliente, correo_agente, estado, firma } = data
          // SI YA TIENE DATOS DE LA FIRMA ENVIAMOS A PROCESAR LOS DATOS
          if (firma) {
            await procesaDatos()
            setEstado(estado)
  ...
  const aprobarFirmaEjecutivo = async (param) => {
    try {
      if (validaFirma()) {
        setIsLoading(true)
        const { data } = await axiosPrivate.post(
          `formulario/graba-ejecutivo/${hash}`,
          param,
        )
  ...
  const firmarEjecutivo = async () => {
    ...
    //
    const { data } = await axiosPrivate.post(
      `formulario/firma-ejecutivo/${hash}`,
      datosFirma,
    )

    if (data) {
      const { correo_cliente, correo_agente, correo_ejecutivo } = data
  ...
  const firmarEjecutivoSinAsesor = async () => {
    ...
    //
    const { data } = await axiosPrivate.post(
      `formulario/firma_cliente_ejecutivo/${hash}`,
      datosFirma,
    )
  ...
  const getDataAsesor = async () => {
    const { formulario } = formData
    console.log('formulario2', formulario.correo_ejecutivo)
    var infoDetail = {
  ...
  return (
    <>
      ...
      >
        ...
        {/* {(estado == 'A' && revisionInternaAsesor) && <Button
        ...
        {(estado == 'A' && revisionInterna || estado == 'E' && revisionInterna) && <Button
