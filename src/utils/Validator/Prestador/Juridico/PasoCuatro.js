...
export const validationSchema = Yup.object().shape({
  ...
  if (values.revisionInterna) {
    return schema.shape({
      ...
      identificacion_ejecutivo: Yup.string()
      .required("Requerido")
  ...
  if(values.revisionInternaBroker){
    return schema.shape({
      ...
      lugar_asesor:Yup.string().required("Requerido"),
      nombre_razonsocial:Yup.string().required("Requerido"),
      observaciones_asesor:Yup.string().required("Requerido"),
    })
  }
}).when((values,schema)=>{
