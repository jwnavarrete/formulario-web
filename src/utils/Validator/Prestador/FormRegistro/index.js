...
export const validationSchema = Yup.object()
  .shape({
    ...
    correo_prestador: Yup.string().trim()
      .required("Requerido")
      .email("el formato debe ser ejemplo: validar@example.com"),
    correo_ejecutivo: Yup.string().trim()
      .required("Requerido")
      .email("el formato debe ser ejemplo: validar@example.com"),
  })
  .when((values, schema) => {
    switch (values.tipo_identificacion) {
      ...
      case "U":
        return schema.shape({
          num_identificacion: Yup.string().trim()
            .required("Requerido")
            .matches(/^\d{13}$/, "Extensión máxima 13 caracteres"),
        });
      case "P":
        return schema.shape({
          num_identificacion: Yup.string().trim()
    ...
    if (values.tipo_prestador.trim() == '04') {
      return schema.shape({
        correo_asesor: Yup.string().trim()
          .required("Requerido")
          .email("el formato debe ser ejemplo: validar@example.com")
      })
    }
